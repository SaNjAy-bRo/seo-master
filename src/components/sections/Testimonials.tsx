        ...
        <section id="testimonials" className="section" style={{ background: "var(--color-bg-orange-soft)", position: "relative", overflow: "hidden" }}>
            ...
            <div className="section-inner" style={{ position: "relative", zIndex: 1 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: "1.5rem" }} className="testimonials-grid">
                    {testimonials.map((t) => (
                        <div key={t.name} className="card" style={{ padding: "2rem", display: "flex", flexDirection: "column", gap: "1.25rem", background: "#fff", boxShadow: "var(--shadow-warm)" }}>
                            {/* Top row — stars + metric */}
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <div style={{ display: "flex", gap: "0.2rem" }}>
                                    {Array.from({ length: t.rating }).map((_, i) => (
                                        <svg key={i} width="18" height="18" viewBox="0 0 24 24" fill="#F97316" stroke="none">
