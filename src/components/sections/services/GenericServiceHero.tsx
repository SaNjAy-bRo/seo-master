...
export default function GenericServiceHero({ label, title, description, checks, children }: ServiceHeroProps) {
    const header = useScrollReveal();
    const btn = useScrollReveal({ delay: 200 });
    const checksReveal = useScrollReveal({ delay: 300 });
    ...
    return (
        ...
        >
            ...
            <div className="section-inner" style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                ...
                <div ref={checksReveal.ref} className={`reveal reveal-up ${checksReveal.isVisible ? "visible" : ""}`} style={{
                    display: "flex",
                    flexWrap: "wrap",
                    gap: "1.5rem",
                    ...
                    {checks.map((check, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "#fff", fontWeight: 600 }}>
                            <div style={{ width: 24, height: 24, borderRadius: "50%", background: "var(--color-primary)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}>
